...
export default function NewProductPage() {
  ...
  async function save(status: ProductStatus) {
    if (!form.name || !form.category || !form.shortDescription || !form.fullDescription) {
      toast.error("Name, category, short description and full description are required");
      return;
    }
    setSaving(true);
    try {
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-2xl font-black text-gray-900 tracking-tight mt-1">New Product</h1>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className={sectionCls}>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              ...
              <input required className={inputCls} placeholder="Fresh Coconuts" value={form.name} onChange={(e) => handleNameChange(e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>URL Slug *</label>
              <input required className={inputCls} placeholder="fresh-coconuts" value={form.slug}
            ...
            <div className="sm:col-span-2">
              ...
              <textarea required rows={2} className={inputCls} placeholder="Brief summary shown in product listings (max 200 chars)" value={form.shortDescription} onChange={(e) => set("shortDescription", e.target.value)} />
            </div>
            <div className="flex items-center gap-6">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-[13px] font-medium text-gray-700">Private Label Available</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className={sectionCls}>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              ...
              <input className={inputCls} placeholder="Cocos nucifera" value={form.scientificName} onChange={(e) => set("scientificName", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>HS Code</label>
              <input className={inputCls} placeholder="0801.19" value={form.hsCode} onChange={(e) => set("hsCode", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Origin</label>
              <input className={inputCls} placeholder="Andhra Pradesh, India" value={form.origin} onChange={(e) => set("origin", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Shelf Life</label>
              <input className={inputCls} placeholder="30–45 days" value={form.shelfLife} onChange={(e) => set("shelfLife", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Lead Time</label>
              <input className={inputCls} placeholder="2–3 weeks" value={form.leadTime} onChange={(e) => set("leadTime", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>MOQ</label>
              <input className={inputCls} placeholder="1 FCL (20ft)" value={form.moq} onChange={(e) => set("moq", e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className={sectionCls}>
          ...
          <p className="text-gray-400 text-[12px] mb-3">HTML supported — headings, lists, paragraphs.</p>
          <textarea rows={8} className={inputCls} placeholder="<h2>About this product</h2><p>Detailed description...</p>" value={form.fullDescription} onChange={(e) => set("fullDescription", e.target.value)} />
        </div>
        ...
        <div className={sectionCls}>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            <div>
              <label className={labelCls}>Available Grades (comma-separated)</label>
              <input className={inputCls} placeholder="Grade A, Export Grade, Premium" value={form.availableGradesText} onChange={(e) => set("availableGradesText", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Certifications (comma-separated)</label>
              <input className={inputCls} placeholder="APEDA, FSSAI, ISO 22000, Organic" value={form.certificationsText} onChange={(e) => set("certificationsText", e.target.value)} />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {specs.map((s, i) => (
              <div key={i} className="flex gap-3 items-center">
                <input className={`flex-1 ${inputCls}`} placeholder="Parameter (e.g., Moisture Content)" value={s.label} onChange={(e) => updateSpec(i, "label", e.target.value)} />
                <input className={`flex-1 ${inputCls}`} placeholder="Value (e.g., Max 12%)" value={s.value} onChange={(e) => updateSpec(i, "value", e.target.value)} />
                <button onClick={() => removeSpec(i)} className="text-gray-400 hover:text-red-500 transition flex-shrink-0">
        ...
        <div className={sectionCls}>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            <div className="sm:col-span-2">
              <label className={labelCls}>Packaging Options (comma-separated)</label>
              <input className={inputCls} placeholder="25kg PP bags, 50kg jute bags, custom packaging" value={form.packagingOptionsText} onChange={(e) => set("packagingOptionsText", e.target.value)} />
            </div>
            <div>
              <label className={labelCls}>Container Capacity</label>
              <input className={inputCls} placeholder="20 MT per 20ft FCL" value={form.containerCapacity} onChange={(e) => set("containerCapacity", e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className={sectionCls}>
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {INCOTERMS.map((t) => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => toggleArr("incotermsSupported", t)}
                    className={`px-3 py-1.5 rounded-full text-[12px] font-bold border transition ${
                      form.incotermsSupported.includes(t)
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {PAY_TERMS.map((t) => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => toggleArr("paymentTerms", t)}
                    className={`px-3 py-1.5 rounded-full text-[12px] font-bold border transition ${
                      form.paymentTerms.includes(t)
            ...
            <div>
              <label className={labelCls}>Export Markets (comma-separated)</label>
              <input className={inputCls} placeholder="Europe, Middle East, North America, Southeast Asia" value={form.exportMarketsText} onChange={(e) => set("exportMarketsText", e.target.value)} />
            </div>
          </div>
        </div>
      </div>
